...
Second, the selectivity panel includes only two human serine hydrolases (trypsin and CES1). A broader off-target profiling panel encompassing CYP450 isoforms, hERG, serum albumin, and other common liability targets was not used in the mechanism-restricted SI, though energies for these targets are reported separately. The absence of hERG and CYP screening is a gap that should be addressed before in vivo studies.
...
Fourth, no experimental validation has been performed. All candidates are purely computational predictions. Enzyme inhibition assays (IC$_{50}$), surface plasmon resonance (SPR) binding studies, and minimum inhibitory concentration (MIC) determination against MRSA strains are essential next steps.

Fifth, the docking energies reported (${-}$9 to ${-}$10~\si{\kcal\per\mol}) are Vina affinity estimates with an expected error of approximately $\pm$\SI{2}{\kcal\per\mol} and should not be interpreted as absolute binding free energies. MM-GBSA rescoring or free-energy perturbation calculations on the top candidates would provide more reliable ranking.

Finally, the compound library (244 BRICS-recombined molecules) is small relative to typical virtual screening campaigns. Expanding the library to thousands of compounds through additional scaffold families or iterative BRICS refinement (via the \texttt{--refine} flag) would likely identify additional candidates and improve hit rates.
