...
- **Free Energy Perturbation (FEP) Resistance Profiling (Phase 4.8)** — **Top-hit only.** When enabled (`use_fep_resistance=True`), replaces heuristic resistance scoring with a rigorous Equilibrium FEP calculation using OpenMM and openmmtools. FEP is only triggered for the top *N* candidates (default: 20, configurable via `fep_top_n`) after docking and MM-GB/SA rescoring. Candidates with >50 heavy atoms or SMILES length >100 are automatically skipped. Uses 11 λ-windows, alchemical decoupling of the ligand, and MBAR free energy estimation to compute ΔΔG between wild-type and mutant receptor binding. Requires `openmm` and `openmmtools`.
...
**FEP is now a top-hit only feature.** It is only triggered for the top *N* candidates (default: 20, configurable via `CONFIG.fep_top_n`) after initial docking and MM-GB/SA rescoring. This avoids computationally prohibitive simulations on less-promising compounds.
